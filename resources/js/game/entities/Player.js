...
export default class Player {
    constructor(scene, x, y) {
        ...
        // Speed boost from waves
        this.waveSpeedBoost = 1;
        this.waveSpeedBoostTime = 0;
    ...
    update(cursors, mobileInput = { x: 0, y: 0 }) {
        ...
        // Update wave speed boost timer
        if (this.waveSpeedBoostTime > 0) {
            this.waveSpeedBoostTime -= this.scene.game.loop.delta;
            if (this.waveSpeedBoostTime <= 0) {
                this.waveSpeedBoost = 1; // Reset to normal speed
            }
        }
    ...
    die(killerName = null) {
        ...
        if (this.scene.echoChannel) {
            this.scene.echoChannel.whisper('playerDied', {
        ...
        this.scene.tweens.add({
            ...
            onComplete: () => {
                ...
                this.scene.showGameOver(killerName);
            }
        });
    }
    ...
    getMaxSpeed() {
        ...
        // Apply wave speed boost if active
        speed *= this.waveSpeedBoost;

        return speed;
    }

    applySpeedBoost(multiplier, duration) {
        this.waveSpeedBoost = multiplier;
        this.waveSpeedBoostTime = duration;
        ...
        this.scene.tweens.add({
            ...
            ease: 'Power2',
            onComplete: () => {
                // Return to normal tint after animation
                this.updateHealthTint();
            }
        });
    ...
    createBoostParticle() {
        ...
        if (speed > 0) {
            ...
            // Use different color if wave boost is active
            const color = this.waveSpeedBoostTime > 0 ? 0x00FFFF : 0x87CEEB;
            const particle = this.scene.add.circle(
